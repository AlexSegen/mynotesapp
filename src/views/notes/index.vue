            ...
            <ul v-else class="list-unstyled p-relative">
                ...
                <template v-else >
                    <router-link class="media" v-for="item in NOTES" :key="item.id" :to="'/notes/' + item._id">
                        <img class="mr-3" :src="item.picture == null ? 'https://via.placeholder.com/64/' + item.category.color.replace('#','') + '/' + item.category.color.replace('#',''): item.picture">
                        <div class="media-body">
                        <h5 class="mt-0 mb-1">{{ item.title }}</h5>
                        <div><span class="badge badge-secondary" :style="'background:' + item.category.color">{{ item.category.title }}</span>. Creada en {{ moment(item.createdAt).format('MMM Do YY') }}</div>
                        </div>
                    </router-link>
                </template>
            </ul>
